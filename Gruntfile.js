...
var _ = require('lodash'),
  ...
  configureGrunt = function(grunt) {
    ...
    grunt.registerTask('validate', 'Run tests and lint code', ['init', 'lint', 'test-all']);
    ...
    //
    grunt.registerTask('test-all', 'Run tests and lint code', ['test-routes', 'test-module', 'test-unit', 'test-integration', 'test-ember', 'test-functional']);

    // ### Lint
    //
    // `grunt lint` will run the linter and the code style checker so you can make sure your code is pretty
    grunt.registerTask('lint', 'Run the code style checks and linter', ['jshint', 'jscs', 'shell:csscomblint']);
    ...
    grunt.registerTask('test-ember', 'Run the ember unit tests', ['test-setup', 'shell:ember:test']);
    ...
    grunt.registerTask('test-functional', 'Run functional interface tests (CasperJS)', ['test-setup', 'shell:ember:dev', 'cleanDatabase', 'express:test', 'spawnCasperJS', 'express:test:stop', 'test-functional-setup']);
    ...
    grunt.registerTask('test-functional-setup', 'Run functional tests for setup', ['test-setup', 'cleanDatabase', 'express:test', 'spawnCasperJS:setup', 'express:test:stop']);
    ...
    grunt.registerTask('coverage-integration', 'Generate unit and integration tests coverage report', ['test-setup', 'mocha_istanbul:coverage_integration']);
    ...
    grunt.registerTask('init', 'Prepare the project for development', ['shell:ember:init', 'shell:bower', 'update_submodules', 'assets', 'default']);

    // ### Basic Asset Building
    // Builds and moves necessary client assets. Prod additionally builds the ember app.
    grunt.registerTask('assets', 'Basic asset building & moving', ['clean:tmp', 'buildAboutPage']);

    // ### Default asset build
    // `grunt` - default grunt task
    //
    // Build assets and dev version of the admin app.
    grunt.registerTask('default', 'Build JS & templates for development', ['shell:ember:dev']);

    // ### Production assets
    // `grunt prod` - will build the minified assets used in production.
    //
    // It is otherwise the same as running `grunt`, but is only used when running Ghost in the `production` env.
    grunt.registerTask('prod', 'Build JS & templates for production', ['shell:ember:prod', 'master-warn']);
    ...
    grunt.registerTask('dev', 'Dev Mode; watch files and restart server on changes', ['bgShell:ember', 'express:dev', 'watch']);
    ...
    grunt.registerTask('release',
      ...
      ' - Zip files in release-folder to dist-folder/#{version} directory', ['init', 'shell:ember:prod', 'clean:release', 'shell:shrinkwrap', 'copy:release', 'compress:release']);

    grunt.registerTask('release-full', 'Create zip package with all needed node modules.', ['init', 'shell:ember:prod', 'clean:release', 'shell:shrinkwrap', 'copy:release', 'compress:release', 'shell:sqlite-bindings', 'copy:full', 'compress:release-full']);
  };
